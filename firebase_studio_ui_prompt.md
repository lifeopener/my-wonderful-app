# Vesper AI Companion OS — Firebase Studio UI 생성 프롬프트

아래 프롬프트를 Firebase Studio의 AI 프롬프트 입력창에 그대로 붙여넣으세요.
...
1. 위 프롬프트를 Firebase Studio 입력창에 **전체 복사**해서 붙여넣기
2. 앱이 생성되면 각 화면별로 아래 추가 프롬프트로 세부 조정 가능:

### 홈 화면 수정 시
```
HomeScreen의 VIX 카드에 실제 수치 애니메이션을 추가해줘. 숫자가 카운트업되면서 등장하도록 해줘.
```

### 채팅 화면 수정 시
```
ChatScreen에서 AI가 타이핑 중일 때 말풍선이 좌측에서 부드럽게 나타나는 애니메이션을 추가해줘.
```

### 페르소나 화면 수정 시
```
PersonaScreen에 페르소나 말투 프리셋 버튼을 3개 추가해줘 — "따뜻한 친구", "냉철한 코치", "유쾌한 멘토"
```
